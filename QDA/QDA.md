...
model_nor <- qda(V1~., data = train_raw.df)
...
#PRROC_obj <- roc.curve(scores.class0 = as.numeric(as.factor(test.lda.predict_nor$class)), weights.class0=as.numeric(as.factor(test_raw.df$V1)),
#                       curve=TRUE)
#pdf(file = "/scratch/user/naminiyakan/hackathon/roc_lda_nor.pdf")
#plot(PRROC_obj)
#dev.off()

library(pROC)
auc(as.numeric(as.factor(test_raw.df$V1)),(test.lda.predict_nor$posterior[,1]))

pdf(file = "/scratch/user/naminiyakan/hackathon/194genes_lda_mil_2500.pdf")
...
ggplot(lda.data, aes(LD1, 0)) +
  geom_point(aes(color = Sev_vs_rest))
dev.off()
